...
import supabaseProvisionRoutes from "./routes/supabaseProvision.js";
...
app.use("/v1/admin", supabaseProvisionRoutes);
...
// In production, require a strong JWT_SECRET (set on the API host env; never commit it).
...
if (isProduction && (!process.env.JWT_SECRET || process.env.JWT_SECRET === devSecret)) {
  console.error("FATAL: Set JWT_SECRET in production (API host environment). Do not use the dev default.");
  process.exit(1);
}
