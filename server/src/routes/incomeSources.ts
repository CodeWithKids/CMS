...
/** GET /v1/income-sources */
router.get("/", async (_req: Request, res: Response) => {
  const rows = await prisma.incomeSource.findMany({ orderBy: { name: "asc" } });
  res.json(rows);
});

/** POST /v1/income-sources - admin only */
router.post("/", requireAuth, async (req: Request, res: Response) => {
  ...
  const id = nextId("inc");
  const row = await prisma.incomeSource.create({ data: { id, name, code } });
  res.status(201).json(row);
});

/** PATCH /v1/income-sources/:id - admin only */
router.patch("/:id", requireAuth, async (req: Request, res: Response) => {
  ...
  if (!existing) {
    sendError(res, 404, "NOT_FOUND", "Income source not found.");
    return;
  }
  const body = req.body ?? {};
  const name = parseString(body.name);
...
/** DELETE /v1/income-sources/:id - admin only */
